...
const Breadcrumb = [
  ...
  { MainComponent: "Photo Gallery ", MainComponentURl: "PhotoGalleryMaster" },
];

const PhotoGalleryForm = ({ item, onCancel, onSave, setLoading }: IPhotoGalleryProps) => {
  const sp: SPFI = getSP();
  ...
  const [existingThumbnails, setExistingThumbnails] = React.useState<{ id: number; name: string; url: string }[]>([]);
  const [existingThumbnailIds, setExistingThumbnailIds] = React.useState<number[]>([]);
...
React.useEffect(() => {
  const fetchExistingThumbnails = async () => {
    ...
    try {
      const listItem = await sp.web.lists
        ...
        .select("Id", "PhotoGalleryID/Id")
        .expand("PhotoGalleryID")();

      if (listItem.PhotoGalleryID?.length > 0) {
        const fileIds = listItem.PhotoGalleryID.map((f: any) => f.Id);
  ...
  if (item) {
    console.log("Editing Photo Gallery item:", item);
    setTitle(item.Title || "");
    ...
    if (departments.length > 0) {
      ...
      if (matchedDept) {
        ...
        console.warn("No matching department found for:", item.Department);
      }
    }
  ...
  const uploadFilesToLibrary = async (files: File[]): Promise<number[]> => {
    const folder = sp.web.getFolderByServerRelativePath("/sites/BAC/PhotoGalleryDocs");
  ...
  const getNewFileName = (fileName: string): string => {
    ...
    const dateRegex = /^\d{8}$/;
    const timeRegex = /^\d{6}$/;
    if (parts.length > 2 && dateRegex.test(parts[0]) && timeRegex.test(parts[1])) {
    ...
    try {
      ...
      if (deletedFileIds.length > 0) {
        for (const fileId of deletedFileIds) {
          await sp.web.lists.getByTitle("PhotoGalleryDocs").items.getById(fileId).delete();
        }
      }
      ...
      const payload: any = {
        ...
        PhotoGalleryIDId: finalFileIds,
      };

      if (item && item.Id) {
        await sp.web.lists.getByTitle("PhotoGallery").items.getById(item.Id).update(payload);
      } else {
        await sp.web.lists.getByTitle("PhotoGallery").items.add(payload);
      }
                ...
                <div className="col-lg-6">
                  <div className="mb-3">
                    <div className="d-flex justify-content-between align-items-center mb-1">
                      ...
                      {(existingThumbnails.length > 0 || thumbnails.length > 0) && (
                        <a
                        ...
                        >
                          ...
                          {existingThumbnails.length + thumbnails.length > 1 ? "files" : "file"} attached
                        </a>
                      )}
                    </div>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            ...
            <p className="text-muted font-14 mb-0 fw-400">Below are the attached files for Photo Gallery</p>
          </Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body>
          <table className="table table-bordered">
            ...
            <tbody>
              {[...existingThumbnails, ...thumbnails].map((file: any, index: number) => {
                const isNewFile = file instanceof File;
                const previewUrl = isNewFile ? URL.createObjectURL(file) : file.url;
