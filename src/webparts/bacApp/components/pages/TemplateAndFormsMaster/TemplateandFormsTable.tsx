...
const TemplateTable = ({ onAdd, onEdit, setLoading }: ITemplateTableProps) => {
  const sp: SPFI = getSP();
  ...
  const Breadcrumb = [
...

    {
      ...
      MainComponentURl: "TemplateandForms",
    },
  ];
  ...
  const getMasterListData = async () => {
    ...
    await sp.web.lists
      ...
      .items.select(
        "*,Title,Department/ID,Department/DepartmentName",
        "Description"
      )
      .expand("Department")
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const item = await sp.web.lists
            .getByTitle("TemplateAndForms")
            .items.getById(id)
            .select("Id", "IconID/Id", "AttachmentID/Id")
            .expand("IconID", "AttachmentID")();

          //Collect all related file IDs
          const attachmentIds = item.AttachmentID
            ? Array.isArray(item.AttachmentID)
              ? item.AttachmentID.map((f: any) => f.Id)
              : [item.AttachmentID.Id]
            : [];

          const iconIds = item.IconID
            ? Array.isArray(item.IconID)
              ? item.IconID.map((f: any) => f.Id)
              : [item.IconID.Id]
            : [];
                    ...
                    <th style={{ minWidth: "120px", maxWidth: "120px" }}>
                      <div className="d-flex flex-column bd-highlight">
                        ...
                        >
                          ...
                          <span onClick={() => handleSortChange("Description")}>
                            <FontAwesomeIcon icon={faSort} />
                          </span>
                        </div>
                        <div className="bd-highlight">
