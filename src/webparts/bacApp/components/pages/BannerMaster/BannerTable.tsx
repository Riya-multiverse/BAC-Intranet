...
const BannerTable = ({ onAdd, onEdit, setLoading }: IMyTableProps) => {
    const sp: SPFI = getSP();
    ...
    const getMasterListData = async () => {
        ...
        //   if (isSuperAdmin == "Yes") {
        await sp.web.lists.getByTitle("Banner").items.select("*,BannerImageID/ID,Department/ID,Department/DepartmentName").expand("BannerImageID,Department").orderBy("Created", false).getAll()
            .then((res) => {
